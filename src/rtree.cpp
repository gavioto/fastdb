...
void dbRtree::traverse(dbDatabase* db, oid_t treeId, dbSearchContext& sc)
{
    dbRtree* tree = (dbRtree*)db->get(treeId);
    if (tree->height == 0) {
        return;
    ...
    rectangle r;
    dbRtreePage::cover(db, tree->root, r);
    Neighbor* list = new Neighbor();
    Neighbor* free = NULL;
    Neighbor *next, *prev;
    dbTable* table = (dbTable*)db->get(sc.cursor->table->tableId);

    list->child = tree->root;
    list->distance = distance(*(rectangle*)sc.firstKey, r);
    list->level = tree->height;
    list->next = NULL;
    ...
    while (true) {
        ...
        oid_t child = neighbor->child;
        int level = neighbor->level;
        list = neighbor->next;
        neighbor->next = free;
        free = neighbor;
        if (level == 0) {
            if (sc.condition != NULL && !db->evaluate(sc.condition, child, table, sc.cursor)) {
                continue;
            }
            if (!sc.cursor->add(child)) {
                break;
            }
            continue;
        }
        dbRtreePage* pg = (dbRtreePage*)db->get(child);
        for (int i = 0, n = pg->n; i < n; i++) {
            neighbor = free;
            if (neighbor == NULL) {
            ...
            neighbor->child = pg->b[i].p;
            neighbor->distance = distance(pg->b[i].rect, *(rectangle*)sc.firstKey);
            neighbor->level = level-1;

            coord_t distance = neighbor->distance;
            for (prev = NULL, next = list; next != NULL && next->distance < distance; prev = next, next = prev->next);
            neighbor->next = next;
            if (prev == NULL) {
                list = neighbor;
            } else {
